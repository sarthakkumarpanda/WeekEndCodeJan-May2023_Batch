		...
		while (iterator.hasNext()) {
			...
			if (!parentWindowHandle.equalsIgnoreCase(ChildWindow)) {
				...
				if (driver.getTitle().equals("Xpath Practice - H Y R Tutorials")) {
					driver.manage().window().maximize();
                    driver.findElement(By.xpath("//div[@class = 'container']/input[1]")).sendKeys("Selenium");
                    driver.findElement(By.xpath("//div[@class = 'container']/input[2]")).sendKeys("Panda");
                    driver.findElement(By.xpath("//div[@class = 'container']/input[3]")).sendKeys("seleniumpanda@gmail.com");
				    driver.close();
				}
